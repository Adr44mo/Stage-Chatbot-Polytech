...
DATA_SITES_DIR = Path(__file__).parent.parent.parent / "Corpus" / "data_sites"
PDF_MAN_DIR = Path(__file__).parent.parent.parent / "Corpus" / "pdf_man"
...
def build_input_maps():
    ...
    for site_dir in DATA_SITES_DIR.iterdir():
        if not site_dir.is_dir() or site_dir.name.lower() == "geipi_polytech" or site_dir.name.lower() == "archives" or site_dir.name.lower() == "polytech_réseau":
            continue
