...
from ..preprocessing.update_map import update_output_maps_entry
...
def detect_type_node(state):
    try:
        ...
        state["traceback"] = traceback.format_exc()
        log_callback(state, "DETECT_TYPE ERROR", color="red")
        raise
...
def end_node(state):
    """
    End node for the graph and put update the jsonmaps
    """

    
    if state.get("is_valid"):
        # Utiliser le chemin du fichier de sortie JSON au lieu du fichier d'entrée
        hash_val = state.get("hash", "")        
        # Ne mettre à jour les maps que pour les fichiers de data_sites
        input_path = Path(state["file_path"])

        ColorPrint.print(f"aaaah {input_path}","red")
