...
def session_id_key(request: Request):
    """
    Retourne le session_id si présent, sinon l’IP.
    """
...
async def chat(request: Request, request_body: ChatRequest, x_session_id: str = Header(...), session: Session = Depends(get_session)):
    """
    ...
    # filtered = handle_if_uninformative(request_body.prompt)
    # if filtered:
